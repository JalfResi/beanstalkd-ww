...
#define CMD_GROUP_CREATE "group-create"
#define CMD_GROUP_STATS "group-stats "
#define CMD_GROUP_DELETE "group-delete "
...
#define CMD_GROUP_CREATE_LEN CONSTSTRLEN(CMD_GROUP_CREATE)
#define CMD_GROUP_STATS_LEN CONSTSTRLEN(CMD_GROUP_STATS)
#define CMD_GROUP_DELETE_LEN CONSTSTRLEN(CMD_GROUP_DELETE)
...
#define MSG_CLEARED_LEN CONSTSTRLEN(MSG_CLEARED)
...
#define OP_GROUP_CREATE 25
#define OP_GROUP_STATS 26
#define OP_GROUP_DELETE 27
#define TOTAL_OPS 28
...
#define STATS_FMT "---\n" \
    ...
    "cmd-group-create: %" PRIu64 "\n" \
    "cmd-group-stats: %" PRIu64 "\n" \
    "cmd-group-delete: %" PRIu64 "\n" \
    "job-timeouts: %" PRIu64 "\n" \
    "total-jobs: %" PRIu64 "\n" \
    "max-job-size: %zu\n" \
    "current-tubes: %zu\n" \
...
{
    if (!j || j->state != JOB_STATE_DELAYED) return NULL;
    j = pq_remove(&j->tube->delay, j);
    if (j) {
        global_stat.pause_ct--;
        j->tube->stat.pause_ct--;
    }
    return j;
}
...
#define TEST_CMD(s,c,o) if (strncmp((s), (c), CONSTSTRLEN(c)) == 0) return (o);
    ...
    TEST_CMD(c->cmd, CMD_GROUP_CREATE, OP_GROUP_CREATE);
    TEST_CMD(c->cmd, CMD_GROUP_STATS, OP_GROUP_STATS);
    TEST_CMD(c->cmd, CMD_GROUP_DELETE, OP_GROUP_DELETE);
    return OP_UNKNOWN;
}
...
fmt_stats(char *buf, size_t size, void *x)
{
    ...
    return snprintf(buf, size, STATS_FMT,
            ...
            op_ct[OP_PAUSE_TUBE],
            op_ct[OP_GROUP_CREATE],
            op_ct[OP_GROUP_STATS],
            op_ct[OP_GROUP_DELETE],
            timeout_ct,
            global_stat.total_jobs_ct,
            job_data_size_limit,
            tubes.used,
...
/* Read a group value from the given buffer and place it in grp. */
static int
read_grp(uint64_t *grp, const char *buf, char **end)
{
    char *tend;
    uint64_t tpri;
...
do_stats(conn c, fmt_fn fmt, void *data)
{
    ...
    char *buf = malloc(40 * 1024);

    /* Writes out the stats if possible into a 
       40kb buffer, if it doesn't fit, we'll re-do
       it later. If it does, we've saved a call! */
    stats_len = fmt(buf, 40 * 1024, data);
	...
	c->out_job->body_size = stats_len;
	
    if(stats_len < 40 * 1024)
	{
	    memcpy(c->out_job->body, buf, stats_len);
	}
	else
	{
	    fmt(c->out_job->body, stats_len, data);
	}
...
static void
do_clear(conn c, tube t)
{
    int r;
    job j;
    
    while((j = pq_peek(&t->ready)))
    {
   ...
   while((j = t->buried.next))
   {
       if (!buried_job_p(t)) break;
    ...
    return reply_line(c, STATE_SENDWORD, "CLEARED\r\n", 9);
}
...
static int
fmt_stats_group(char *buf, size_t size, group *g)
{
	int ret;
	
	if(g->total > 0)
	{
		char *jobs = malloc(sizeof(uint64_t) * (g->total + (g->total - 1)));
		memset((void *)jobs, 0x00, sizeof(uint64_t) * (g->total + (g->total - 1)));
		...
		{
			char str[33];
			sprintf(str, "%lu", g->jobs[i]);
			
			strncat(jobs, str, sizeof(uint64_t));
			
			if(i < g->total - 1)
				strncat(jobs, sep, sizeof(char));
		}
		
		free(sep);
		
		ret = snprintf(buf, size, STATS_GROUP_FMT,
				g->id,
				g->total,
				g->finished,
				g->ready,
...
dispatch_cmd(conn c)
{
    ...
    job j;
    unsigned char type;
    char *size_buf, *delay_buf, *ttr_buf, *pri_buf, *end_buf, *name, *grp_buf;
    unsigned int pri;
    usec delay, ttr;
    uint64_t id, grp, body_size;
    tube t = NULL;
    group *g = NULL;
    ...
    case OP_PUT:
        ...
        r = read_ttr(&ttr, ttr_buf, &grp_buf);
        if (r) return reply_msg(c, MSG_BAD_FORMAT);

        r = read_grp(&grp, grp_buf, &size_buf);
        if (r) return reply_msg(c, MSG_BAD_FORMAT);

        errno = 0;
        body_size = strtoull(size_buf, &end_buf, 10);
        ...
        c->in_job = make_job(pri, delay, ttr ? : 1, body_size + 2, c->use, g);
    ...
    case OP_GROUP_CREATE:
        ...
        job_id_str = c->cmd + CMD_GROUP_CREATE_LEN;
        
		int total;
        uint64_t *jobs = explode(job_id_str, ',', &total);
        ...
        uint64_t id = create_group(jobs, total);
        op_ct[type]++;
        
        return reply_line(c, STATE_SENDWORD, "CREATED GROUP %u\r\n", id);
        
    	break;
    case OP_GROUP_STATS:
        errno = 0;
        id = strtoull(c->cmd + CMD_GROUP_STATS_LEN, &end_buf, 10);
        if (errno) return reply_msg(c, MSG_BAD_FORMAT);
        op_ct[type]++;
        
        g = find_group(id);
        
        if (g == NULL) return reply_msg(c, MSG_NOTFOUND);
        g->finished = 0;
        g->reserved = 0;
        g->ready = 0;
        g->buried = 0;
        ...
        do_stats(c, (fmt_fn) fmt_stats_group, g);
        
    	break;
    case OP_GROUP_DELETE:
        errno = 0;
        id = strtoull(c->cmd + CMD_GROUP_DELETE_LEN, &end_buf, 10);
        if (errno) return reply_msg(c, MSG_BAD_FORMAT);
        op_ct[type]++;
        
        int ret = delete_group(id);
        
        if(ret == 0) return reply_msg(c, MSG_NOTFOUND);
        
        return reply_line(c, STATE_SENDWORD, "DELETED GROUP %lu\r\n", id);
